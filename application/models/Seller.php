...
/**
 * Seller Model for bs_sellers table
 */
class Seller extends PS_Model {
	...
	function __construct() 
	{
        ...
        parent::__construct( 'bs_sellers', 'seller_id', 'slr' );
	...
	function custom_conds( $conds = array())
	{
		...
		if ( isset( $conds['order_by'] )) {
			...
			$this->db->order_by( 'bs_sellers.'.$order_by_field, $order_by_type);
		}

		// seller_id condition
		if ( isset( $conds['seller_id'] )) {
			$this->db->where( $this->primary_key, $conds['seller_id'] );
		}
		...
		if ( isset( $conds['searchterm'] )) {
			$this->db->like( 'seller_name', $conds['searchterm'] );
			$this->db->or_like( 'seller_email', $conds['searchterm'] );
		}
	 ...
	 * @return bool
	 */
	function save_seller( &$seller_data, $permission_data, $seller_id = false )
	{
		// start the transaction
		$this->db->trans_start();

		if ( !$seller_id && ! $this->exists( array( 'seller_email' => $seller_data['seller_email'] ))) {
		//if there is no data with this id, insert new			
			
			// generate new user id
			$seller_id = $this->generate_key( 'USR' );
			$seller_data['seller_id'] = $seller_id;

			if ( ! $this->db->insert( $this->table_name, $seller_data )) {
		...
		//else update the data
			...
			if ( ! $this->db->update( $this->table_name, $seller_data )) {
		...
		if ( ! $this->db->delete( $this->permission_table_name, array( $this->primary_key => $seller_id )) ) {
		...
		foreach ( $permission_data as $module ) {
			...
			// prep module data
			$data = array( 'module_id' => $module, $this->primary_key => $seller_id );
